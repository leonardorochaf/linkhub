import { UserProfile, Link as LinkType } from '../store/linkStore';
...
const ProfilePreview = ({ profile, links }: ProfilePreviewProps) => {
  const activeLinks = links.filter(link => link.active);

  return (
    <div className={`h-full flex flex-col items-center py-10 px-4 ${profile.theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-muted/20'}`}>
